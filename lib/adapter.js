    ...
    find: function (connection, collection, options, cb) {
		...
		mssql.connect(marshalConfig(connections[connection].config), function __FIND__(err) {
			...
			request.query(statement, function(err, recordset) {
				...
				if (err) return cb(err);
				cb(null, sanitizer.sanitizeResultSet(recordset));
      ...
      if (_.isFunction(data)) {
        ...
        data = null;
     	...
     	mssql.connect(marshalConfig(connections[connection].config), function __FIND__(err) {
			...
			var request = new mssql.Request();
      if(data && data.output){
        request.input('param_CUSTOMER_ID', mssql.Int, 57791);
        request.input('param_USER_ID', mssql.Int, 0);

        request.output('output_IS_SUCCESSFUL', mssql.VarChar(500));
        request.output('output_STATUS', mssql.VarChar(500));

        request.execute(query, function(err, recordsets, returnValue) {

          if (err) return cb(err);
          cb(null, {
            recordSets: recordsets,
            returnValue: returnValue,
            outputs: request.parameters
          });
        });
