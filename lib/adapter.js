    ...
    query: function(connection, collection, query, data, cb) {
      ...
      if(data && data.output){
        _.each(data.input, function(input) {
          request.output(input.name, input.val);
        });
        _.each(data.output, function(output) {
          request.output(output.name, output.val);
        });
        request.execute(query, function(err, recordsets, returnValue) {
          ...
          if (err) return cb(err);
          cb(null, {
            recordSet: sanitizer.sanitizeResultSet(recordset),
            returnValue: sanitizer.sanitizeResultSet(returnValue)
          });
        });
